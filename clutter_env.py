...
@register_env("TabletopClutter-v1", max_episode_steps=200)
class TabletopClutterEnv(BaseEnv):
    DEFAULT_EPISODE_JSON: str = r"def_ep.json"
    def __init__(
        self,
        *args,
        robot_uids="panda",
    ...
    def _default_sensor_configs(self):
        pose = sapien_utils.look_at(eye=[0.1, 0, 0.6], target=[-0.1, 0, 0.1])
        return [
            CameraConfig(
                "base_camera",
                pose=pose,
    ...
    def _load_agent(self, options: dict):
        super()._load_agent(options, sapien.Pose(p=[-0.3, 0, 0.1]))

        robot = self.agent.robot

        wrist_link = None
        for link in robot.get_links():
            if link.get_name() == "right_base2_link":
                wrist_link = link
                break
    ...
    def get_reward(self, obs=None, action=None, info=None):
        return 0.0